...
/**
 * Determina a posição do peso na cama com base nos 4 sensores
 */
String determinar_posicao(float s1, float s2, float s3, float s4, float total) {
...
/**
 * Verifica se há risco de queda com base nas porcentagens dos quadrantes
 * Retorna o código do tipo de risco (0 = sem risco)
 */
int verificar_tipo_risco() {
...
/**
 * Ajusta o número de confirmações necessárias para detectar risco
 */
void ajustar_confirmacoes_risco(int delta) {
...
/**
 * Lê o peso com estabilização por média móvel
 */
float ler_peso_estabilizado(HX711 &sensor, float *leituras, int &indice, float &total, bool &inicializado) {
...
/**
 * Envia posição via UART para o módulo FTDI
 */
void enviar_posicao_ftdi(String posicao) {
...
/**
 * Reduz a frequência da CPU para economizar energia
 */
void reduzir_frequencia_cpu() {
